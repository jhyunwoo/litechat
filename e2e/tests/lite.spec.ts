...
test('전체 여정: 가입 → 친구 → 실시간 채팅 → 읽음 → 이미지 탭투로드 → 데이터 카운터', async ({
  ...
  // ── 데이터 사용량 카운터가 표시된다 (0이 아닌 값) — 상태줄 우측
  const usage = alice.locator('.use').first();
  ...
  await alice.getByRole('button', { name: '추가' }).click();
  ...
  await alice.locator('.bar textarea').fill('라이트에서 안녕!');
  await alice.locator('.bar textarea').press('Enter');
  await expect(alice.locator('.msgs').getByText('라이트에서 안녕!')).toBeVisible();
  ...
  await expect(bob.locator('.msgs').getByText('라이트에서 안녕!')).toBeVisible();
  ...
  await expect(bob.locator('.cb img')).toBeVisible();
  // 이미지 탭 → 원본 다운로드 오버레이
...
test('작성 중 초안이 수신 메시지 재렌더에도 보존된다', async ({ browser }) => {
  const aliceName = uniqueName('ld');
  const bobName = uniqueName('le');
  const aliceContext = await browser.newContext({ baseURL: BASE });
  const bobContext = await browser.newContext({ baseURL: BASE });
  const alice = await aliceContext.newPage();
  const bob = await bobContext.newPage();

  await register(alice, aliceName, '초안A');
  await register(bob, bobName, '초안B');
  ...
  // 서로 채팅방 진입
  await expect(alice.getByText('초안B')).toBeVisible();
  await alice.getByText('초안B').last().click();
  await bob.getByText('초안A').last().click();

  // bob이 작성 중인 상태에서 alice가 연달아 메시지를 보낸다 (ack/수신 프레임이 재렌더 유발)
  const bobInput = bob.locator('.bar textarea');
  await bobInput.fill('작성 중인 답장');
  await alice.locator('.bar textarea').fill('첫번째');
  await alice.locator('.bar textarea').press('Enter');
  await alice.locator('.bar textarea').fill('두번째');
  await alice.locator('.bar textarea').press('Enter');
  await expect(bob.locator('.msgs').getByText('두번째')).toBeVisible();

  // 재렌더 후에도 bob의 초안이 살아 있고 그대로 전송된다
  await expect(bob.locator('.bar textarea')).toHaveValue('작성 중인 답장');
  await bob.locator('.bar textarea').press('Enter');
  await expect(alice.locator('.msgs').getByText('작성 중인 답장')).toBeVisible();
