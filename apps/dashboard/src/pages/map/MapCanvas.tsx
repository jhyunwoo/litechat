 ...
 *    줌을 바꿔도 항상 실제 지리 반경(Insights 우선, GeoLite2 폴백)을 나타낸다 — 예전
 *    픽셀 고정 div 마커가 줌마다 반경이 달라 보이던 문제의 해결책. 접속 수는 원의
 *    채움 불투명도와 중앙 카운트 칩으로 인코딩한다.
 * 2. 접속 기록 핀: 현재 패널에 나열된 기록 중 위치가 있는 것들. 저장된 Insights
 *    위치가 있으면 그것을, 없으면 GeoLite2 위치를 사용한다. 선택된 기록은 브랜드
 *    인디고 + 확대로 강조되어 어느 핀인지 바로 보인다.
  ...
  return (
    <APIProvider apiKey={config.googleMapsApiKey}>
      ...
      >
        ...
        {/* 접속 기록 핀 — row 좌표는 서버에서 Insights 우선으로 계산되어 있다. */}
