...
export function errorMessage(error: unknown): string {
  ...
  const messages: Record<string, string> = {
    ...
    BODY_TOO_LARGE: '파일이 너무 커요. 10MB 이하의 이미지를 선택해 주세요.',
    UPLOAD_BUSY: '사진을 처리 중이에요. 잠시 후 다시 시도해 주세요.',
    UPLOAD_QUOTA_EXCEEDED: '사진 저장 용량을 초과했어요.',
    INVALID_PUSH_ENDPOINT: '이 브라우저의 알림 서비스를 지원하지 않아요.',
